...
chrome.runtime.onMessage.addListener((message) => {
  ...
  switch (event) {
    case "OnStartLinkAnalysis":
      ...
      processAnalyze(data.url, data.user_agent);
      break;
...
const processAnalyze = async (url, user_agent) => {
  ...
  if (baseUrl) {
    ...
    console.log(baseUrl, user_agent);

    const data = await postToAnalyze(baseUrl, user_agent);
